...
from transformers import AutoModelForCausalLM, OlmoeConfig, AutoTokenizer
...
def add_arguments(parser):
    group = parser.add_argument_group(title='OlmoE saver.')
...
def save_checkpoint(queue: mp.Queue, args):
    ...
    for i_layer in range(hf_config.num_hidden_layers):
        ...
        # patch missing dimensions
        q_norm_size = message["q_norm"].shape[0]
        set_hf_param(suffix + "self_attn.q_norm", message["q_norm"].repeat(hf_config.hidden_size // q_norm_size))
        set_hf_param(suffix + "self_attn.k_norm", message["k_norm"].repeat(hf_config.hidden_size // q_norm_size))
        set_hf_param(suffix + "mlp.gate", message["mlp gate weight"])
        for expert in range(hf_config.num_experts):
            set_hf_param(suffix + f'mlp.experts.{expert}.gate_proj', message[f"mlp expert{expert} l0 weight W"])
            set_hf_param(suffix + f'mlp.experts.{expert}.up_proj', message[f"mlp expert{expert} l0 weight V"])
            set_hf_param(suffix + f'mlp.experts.{expert}.down_proj', message[f"mlp expert{expert} l1 weight"])
