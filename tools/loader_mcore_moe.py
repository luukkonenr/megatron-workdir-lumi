        ...
        for pp_rank in range(pp_size):
            ...
            for layer_num in range(schema.get_num_layers(models[0])):
                ...
                if margs.qk_layernorm:
                    message["q_norm"] = layer["self_attn_q_norm"]
                    message["k_norm"] = layer["self_attn_k_norm"]
                ...
                for tp_rank, model in enumerate(models):
                    ...
                    # check for experts
                    if margs.num_experts > 1:
                        mlp_l0_weight.append( [ layer[f"mlp_fc1_weight.{idx}"] for idx in range(margs.num_experts)] )
                        mlp_l1_weight.append( [ layer[f"mlp_fc2_weight.{idx}"] for idx in range(margs.num_experts)] )
                    else: 
                ...
                if md.swiglu:
                    ...
                    if margs.num_experts > 1:
                        for expert in range(margs.num_experts):
                            expert_l0_weight = [torch.chunk(mlp_l0_weight[tp_rank][expert], 2, dim=0) for tp_rank in range(tp_size)]
                            message[f"mlp expert{expert} l0 weight W"] = torch.cat([w[0] for w in expert_l0_weight], dim=0)
                            message[f"mlp expert{expert} l0 weight V"] = torch.cat([w[1] for w in expert_l0_weight], dim=0)
                ...
                if margs.num_experts > 1:
                    for expert in range(margs.num_experts):
                        message[f"mlp expert{expert} l1 weight"] = torch.cat([mlp_l1_weight[tp_rank][expert] for tp_rank in range(tp_size)], dim=1)
                    message[f"mlp gate weight"] = layer["router_weight"]
                else:
