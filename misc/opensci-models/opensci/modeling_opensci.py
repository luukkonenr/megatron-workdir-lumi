...
_CHECKPOINT_FOR_DOC = "Qwen/Opensci-8B"
...
class OpensciAttention(nn.Module):
    ...
    def __init__(self, config: OpensciConfig, layer_idx: int):
        ...
        self.q_norm = OpensciRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # unlike olmo, only on the head dim!
        self.k_norm = OpensciRMSNorm(self.head_dim, eps=config.rms_norm_eps)  # thus post q_norm does not need reshape
            ...
            if attention_mask is not None and past_key_values is not None:
                ...
                if is_padding_right:
                    raise ValueError(
                        ...
                        " this may lead to unexpected behaviour for Flash Attention version of Opensci. Make sure to "
...
class OpensciForCausalLM(OpensciPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        >>> model = OpensciForCausalLM.from_pretrained("Qwen/Opensci-8B")
        >>> tokenizer = AutoTokenizer.from_pretrained("Qwen/Opensci-8B")
...
@add_start_docstrings(
    """
    The Opensci Model transformer with a token classification head on top (a linear layer on top of the hidden-states
...
@add_start_docstrings(
    """
The Opensci Model transformer with a span classification head on top for extractive question-answering tasks like
...
class OpensciForQuestionAnswering(OpensciPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.transformer = OpensciModel(config)
